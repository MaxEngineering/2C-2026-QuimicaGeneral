# Clase 2 — Explicación completa, diapositiva por diapositiva
...
**Tema de la clase:** Uniones Químicas · Electrones de valencia · Teoría de Lewis · Regla del octeto y excepciones · Resonancia · Cargas formales · Geometría molecular (TRePEV)
...
⚠️ Ojo con el **Br**: los $3d^{10}$ **NO** cuentan como valencia aunque tengan `n=3`. La capa `d` ya está completa y pertenece al core. La valencia son sólo los $4s^2\,4p^5$ = **7 electrones**. Regla práctica: para los elementos representativos (bloques s y p), **el número de electrones de valencia = número de grupo** (contando 1–18: grupo − 10 para el bloque p, ver P2). Br está en el grupo 17 → 7 e⁻ de valencia. ✓
...
## P1 — pág. 4 · Teoría de Lewis — Moléculas y enlace (hipótesis)

Recuadro: **Lewis: "átomos comparten pares de electrones externos"**. Se basa en estas hipótesis:

- Los átomos, para conseguir **8 e⁻** en su última capa, comparten tantos electrones como les falten para completar su capa (**regla del octeto**).
- Cada **pareja de e⁻ compartidos forma un enlace**.
- Se pueden formar enlaces **sencillos, dobles y triples** con el mismo átomo.

**Explicación:** acá está la idea central del **enlace covalente**: dos átomos ponen electrones en común (los **comparten**) y ese par compartido los mantiene unidos. ¿Por qué comparten? Para llegar a tener **8 electrones** en la última capa (configuración de gas noble, que es la más estable). Si un átomo comparte 1 par → enlace **simple**; 2 pares → **doble**; 3 pares → **triple**.

---

## P1 — pág. 5 · Moléculas y enlace — H₂ y F₂

Se muestran dos moléculas, cada una en tres representaciones (átomos con puntos → nubes solapadas → fórmula desarrollada):
...
## P1 — pág. 6 · Moléculas y enlace — Fórmula desarrollada

Texto: **Fórmula desarrollada**: representa mediante un **guión** los pares de e⁻ que participan de la unión covalente.

Se ve el pasaje: la estructura de puntos de H₂ (`H•` `•H`) → con la flecha → **H—H** (recuadrado en rojo). Debajo repite F₂ con CE(F) $=1s^2 2s^2 2p^5$.

**Explicación:** convención de notación. En vez de dibujar los dos puntitos del par de enlace, se reemplaza por una **línea (guión)**. Un guión = un par compartido = un enlace. Los **pares libres** se siguen dibujando como puntitos. Es la forma estándar y la que conviene usar para no marearse con tantos puntos.
...
**Explicación:** hay que distinguir bien los dos tipos de par, porque más adelante (en TRePEV) los **pares libres empujan más fuerte** que los pares de enlace y son los que deforman la geometría. En F₂: 1 par de enlace + 3 pares libres en cada F. Cada F: 1 par de enlace + 3 pares libres = 4 pares = **8 electrones** ✓ (octeto cumplido).

---

## P1 — pág. 8 · Reglas del octeto

$$\text{Pares de e}^-\text{ de enlace} + \text{Pares de e}^-\text{ no compartidos} = \boxed{4}$$
...
## P1 — pág. 9 · Enlaces simples o múltiples (F₂ y O₂)
...
## P1 — pág. 10 · Enlace triple (N₂)

Diapositiva casi vacía: título **Enlace triple** y la fórmula **N₂** grande en rojo.

**Explicación:** es una diapositiva "para completar en el pizarrón". El **N** tiene 5 e⁻ de valencia ($2s^2\,2p^3$), le faltan **3** para el octeto → los dos N comparten **3 pares** → **enlace triple**:

$$:N\equiv N:$$

Cada N queda con: 3 pares de enlace (el triple) + 1 par libre = 4 pares = **8 e⁻** ✓. El triple enlace del N₂ es uno de los enlaces más fuertes que existen, por eso el N₂ es un gas tan inerte (cuesta muchísimo romperlo).
...
## P1 — pág. 12 · Amoníaco (paso 3: contar electrones de valencia)

3. **Contar la cantidad de electrones externos** de cada elemento (sumar o restar si hay carga). **Dividir por dos** para tener la cantidad de **pares** a ubicar.

$$CEE(N) = 2s^2\,2p^3 \rightarrow 5\ e^-\text{ de valencia}$$
$$CEE(H) = 1s^1 \rightarrow 1\ e^-\text{ por átomo de H} \rightarrow 3\ H = 3\ e^-$$

**Total** = 5 + 3 = **8 e⁻** de valencia = **4 pares de e⁻**.

**Explicación:** este es el paso clave y el que más se equivoca la gente. Sumás **todos** los electrones de valencia de **todos** los átomos de la molécula. N aporta 5, cada H aporta 1 (y hay tres). Total 8 → **4 pares**. Ese número (4 pares) es el "presupuesto" de electrones que vas a repartir en toda la estructura. No podés poner ni más ni menos.

---

## P1 — pág. 13 · Amoníaco (paso 4: formar las uniones)
...
**Explicación:** poné un enlace simple (1 par = 2 e⁻) entre el N y cada H. Son 3 enlaces N–H → **6 electrones gastados** (3 pares). Del presupuesto de 8 e⁻ (4 pares), ya usaste 3 pares; **te queda 1 par (2 e⁻)** por colocar.

---

## P1 — pág. 14 · Amoníaco (paso 5: completar octetos)

5. **Completar el octeto** de todos los átomos externos con dos puntitos (cada uno = un electrón).

Estructura final: el N con **un par libre arriba** (:) y sus 3 enlaces a los H.
...
## P1 — pág. 15 · Estructuras de Lewis — Ion sulfito SO₃²⁻ (pasos 1 y 2)
...
## P1 — pág. 16 · Ion sulfito SO₃²⁻ (pasos 3 y 4)

3. **Formá las uniones:** S en el centro con 3 O alrededor (uno arriba, dos a los lados), tres enlaces simples S–O.
...
Estructura final entre **corchetes con carga 2−**: cada O con 3 pares libres, el S con 1 par libre.
...
**Explicación:** diapositiva de transición para mostrar que las uniones **múltiples** son comunes. Fijate el **SO₃**: ya aparecen las **cargas formales** rotuladas (S⁺², O⁻¹) que se ven en detalle en el P2. Estos modelos 3D anticipan que la molécula tiene una **forma en el espacio** — el tema de la segunda mitad de la clase.
...
3. Formá las uniones.
4. Acomodá los electrones no compartidos.
...
**Explicación:** SO₂ tiene S (6 e⁻) + 2 O (12 e⁻) = **18 e⁻ = 9 pares**. Hacés 2 enlaces simples S–O (2 pares) → quedan **7 pares**. Si repartís esos 7 pares como pares libres (3 a cada O = 6, y 1 al S), el **S se queda con sólo 6 electrones** (2 enlaces + 1 par libre). No llega al octeto. Esto es lo que motiva el paso siguiente.

---

## P1 — pág. 19 · SO₂ (resolución: doble enlace)
...
**Explicación:** la regla de rescate del octeto. Si al átomo central le faltan electrones, **no** se inventan electrones nuevos: se toma un **par libre de un O periférico** y se convierte en un **segundo enlace** (doble) con el S. Al compartir ese par, ahora el **S** también lo cuenta → llega a 8. Resultado: SO₂ tiene un enlace **doble** (S=O) y uno **simple** (S–O)... lo que abre la puerta a la **resonancia** (¿cuál O tiene el doble?), tema que se retoma en P3.
...
- Formá las uniones / Acomodá los no compartidos.

Dos estructuras: la desarrollada **F—B—F** con un tercer F abajo (sin puntos), y la completa con cada F rodeado de 3 pares libres.
...
**Explicación:** el **Boro** es el ejemplo estrella de **átomo deficiente**. Tiene 3 e⁻ de valencia, hace 3 enlaces B–F y... se queda con **sólo 6 electrones** (3 pares de enlace, sin par libre). **No llega al octeto y está bien así**: el B es del grupo 13 y es estable con 6. Total de valencia: B(3) + 3×F(7) = **24 e⁻ = 12 pares**. 3 pares en los enlaces + 9 pares libres (3 por cada F) = 12 ✓.
...
## P1 — pág. 22 · Excepciones al octeto — Expansión: SF₄

Título azul: **Expansión del octeto: Tetrafluoruro de Azufre, SF₄**.

- Átomo central = **S**.
- Electrones de valencia = **34** o **17 pares**.
- Formá las uniones / Acomodá los no compartidos.

Dos dibujos: la estructura con 4 enlaces S–F (**F—S—F** con dos F más) y la completa con pares libres en los F y **un par libre sobre el S**.

**Explicación:** el otro tipo de excepción: **octeto expandido**. El S está en el **período 3**, así que puede usar orbitales `d` cercanos para acomodar **más de 8 electrones**. En SF₄ el S hace **4 enlaces S–F** y además le queda **1 par libre** → 4 pares de enlace + 1 par libre = **5 pares = 10 electrones** alrededor del S. Total valencia: S(6) + 4×F(7) = **34 e⁻ = 17 pares**. Esos 5 pares alrededor del S son los que dan la geometría "sube y baja" que aparece en la tabla final.

---

## P1 — pág. 23 · Geometría Electrónica y Molecular — TRePEV (portada)

Diapositiva de título: **Geometría Electrónica y Molecular** — **TRePEV**.

**Explicación:** cambia el tema. Hasta acá aprendimos a dibujar la estructura de Lewis (la **conectividad**: quién se une con quién). Pero Lewis **no dice nada sobre la forma en el espacio**. Para eso está la **TRePEV** (Teoría de Repulsión de Pares de Electrones de Valencia), que predice la **geometría** de la molécula.

---

## P1 — pág. 24 · Geometría Molecular — qué es TRePEV
...
**Explicación:** repite y formaliza lo de la diapositiva anterior. Lo importante es la frase en rojo: **de a pares, lo más alejados posible**. Ese es el único principio del que sale todo. La cantidad de "cosas" alrededor del central manda la forma.

---

## P1 — pág. 26 · TRePEV — postulados

**Predice geometría molecular.** Postulados:

1. Se tiene en cuenta el **número de grupos electrónicos** alrededor del átomo central (pares no compartidos o enlaces).
2. Los pares electrónicos **se repelen entre sí**. Se orientan para ocupar las posiciones a **mayor distancia posible**, reduciendo al mínimo la repulsión → mayor estabilidad.
3. Los pares de electrones **no compartidos** se repelen con **mayor intensidad** que los pares compartidos → ocupan más espacio.
...
- ✓ Determinar la **geometría molecular** teniendo en cuenta la posición de los átomos (recordar: pares libres → mayor repulsión).
...
4. Mirando sólo **dónde están los átomos** (ignorando los pares libres) sale la **geometría molecular** (la forma "real" de la molécula).

⚠️ **Geometría electrónica ≠ geometría molecular.** La electrónica cuenta pares libres + enlaces; la molecular cuenta sólo átomos. Cuando no hay pares libres en el central, coinciden. Cuando hay pares libres, **difieren**.

---

## P1 — pág. 28 · Por ejemplo… CCl₄

Tres representaciones del **CCl₄** (tetracloruro de carbono): la estructura de Lewis (C central, 4 Cl con sus pares libres), el modelo de bolas y palos con el ángulo **109,5°** marcado, y el modelo compacto (espacial).

**Explicación:** ejemplo de geometría **tetraédrica** perfecta. El C tiene **4 regiones** (4 enlaces C–Cl, **cero pares libres**). Cuatro nubes que se repelen se acomodan apuntando a los vértices de un **tetraedro**, con ángulos de **109,5°**. Como no hay pares libres, geometría electrónica = geometría molecular = **tetraédrica**. Es la forma más importante de recordar porque el carbono la usa en casi toda la química orgánica.
...
Abajo (fondo negro), las tres estructuras de Lewis: H–C(H)(H)–H, H–N̈–H (con par libre) y H–Ö–H (con dos pares libres).
...
**Explicación:** confirma lo anterior. Contando **todas** las regiones (enlaces + pares libres), las tres tienen 4 → **geometría electrónica tetraédrica** en las tres. La diferencia va a estar en qué nubes son átomos y cuáles son pares libres invisibles.
...
**Explicación:** acá está el punto fino. Aunque las tres tengan disposición electrónica tetraédrica, la **forma de la molécula** (lo que ves si sólo mirás los átomos) es distinta, porque en NH₃ y H₂O algunos vértices del tetraedro están ocupados por **pares libres invisibles**, no por átomos. Un par libre "está" pero no lo ves como forma.
...
⚠️ Fijate cómo **baja el ángulo** a medida que hay más pares libres (109,5 → 107,3 → 104,5). Es el **postulado 3** en acción: cada par libre empuja más fuerte y "aprieta" a los enlaces, cerrando el ángulo. Esto se pregunta en el parcial: saber justificar por qué el ángulo del agua es menor que el del metano.
...
## P1 — pág. 34 · Geometrías SIN pares libres (4 y 5 regiones)
...
## P1 — pág. 35 · Geometrías SIN pares libres (6 regiones)
...
## P1 — pág. 36 · Geometrías CON pares de electrones libres (parte 1)
...
- **AB₄E** (SF₄): base bipiramidal, un vértice es par libre → **balancín / "sube y baja"**.

---

## P1 — pág. 37 · Geometrías CON pares libres (parte 2)
...
El **modelo de Lewis** de enlace químico usa el concepto de **enlace covalente** (**co-valente** = **co**mparten la **valen**cia) para indicar que en una unión química se comparten los electrones de valencia. Tres postulados:

1. Los únicos electrones que se consideran son los **de valencia**. Su número está relacionado con el **grupo** de la tabla periódica: con la numeración de grupos "G = 1–18", los elementos del **bloque s y d** tienen **G** electrones de valencia, mientras que los del **bloque p** tienen **G − 10**.
2. El enlace covalente está formado por **pares de electrones compartidos** entre átomos adyacentes. Los pares no compartidos se llaman **pares libres**.
3. **Regla del octeto:** los elementos del bloque p, al formar parte de una molécula, buscan conseguir **8 electrones de valencia** (la configuración del gas noble más cercano). El **H forma duetos** (2 e⁻). Existen excepciones: **octeto expandido** (período ≥ 3) y **octeto incompleto** (como el Boro, 6 e⁻).

**Nota para recordar:** la estructura de Lewis **NO** indica nada sobre el arreglo espacial (geometría); sólo informa **cómo se unen los átomos** (la conectividad).

**Explicación:** es la versión "formal y escrita" de las diapos P1 págs. 2–8. Lo nuevo y útil acá es la **regla del grupo** para contar valencia sin hacer la configuración: para el bloque p, electrones de valencia = **número de grupo − 10** (ej.: O está en el grupo 16 → 16 − 10 = 6 e⁻ ✓; F grupo 17 → 7 ✓). Y la aclaración de que **Lewis no dice nada de geometría** (por eso hace falta TRePEV).

---

## P2 — pág. 2 · Método paso a paso — pasos 1 y 2 (tabla BF₃, XeF₄, NO)
...
**Explicación:** el método completo, idéntico al de P1 pero con el detalle del **caso impar** (NO): 11 electrones no se pueden agrupar en pares exactos, queda **5 pares + 1 electrón solo (desapareado)**. Las moléculas con electrones impares (radicales) **no pueden cumplir el octeto en todos los átomos** — el NO es el ejemplo clásico.
...
Es la contracara escrita de las diapos P1 20–22, con el agregado de por qué en BF₃ no se fuerza el doble enlace (lo justifica con cargas formales en la pág. 5–6).

---

## P2 — pág. 4 · Estructuras finales · Resonancia · Iones (pasos 7 y 8)

Se muestran las estructuras finales de **BF₃** (B con 6 e⁻), **XeF₄** (Xe con 4 enlaces + 2 pares libres) y **NO** (con el electrón desapareado sobre el N).

**Nota para recordar:** si el número total de electrones externos es **impar**, **nunca** se podrá cumplir el octeto para todos los átomos.

**7. Resonancia:** considerar **TODAS** las estructuras de Lewis que describen a la molécula. El conjunto forma el **híbrido de resonancia**. Estas estructuras difieren **sólo en la localización de los electrones**, no en la conectividad. Cuál es la más significativa se decide con las **cargas formales**.

**8. Iones:** se coloca la estructura entre **corchetes** con la **carga total como superíndice**. Si hay varias estructuras de resonancia, van entre corchetes conectadas por una **flecha de doble punta** (↔). Ejemplo: el **NO₃⁻** con sus 3 estructuras resonantes.

**Explicación:** introduce los dos conceptos finales. **Resonancia**: cuando hay más de una forma válida de dibujar la molécula (el doble enlace puede ir en distintos lugares), la realidad es una **mezcla/promedio** de todas (el híbrido). No es que la molécula "salte" entre formas: es un promedio permanente. Y la notación de iones: **corchetes + carga**, con **↔** entre resonantes (⚠️ la flecha ↔ NO es una reacción, es "es lo mismo que").

---

## P2 — pág. 5 · Cargas formales — Conceptos principales

Cuando hay **varias** estructuras de Lewis que cumplen el octeto, ¿cuál es la correcta? Criterio: la estructura que **minimice la carga formal** de cada átomo.

**Definición:** la **carga formal (CF)** de un átomo es la diferencia entre el número de electrones de valencia del **átomo aislado** y el número de electrones **asignados** a ese átomo en la estructura de Lewis.
...
4. Las cargas **negativas** deben quedar en los átomos **más electronegativos**.
5. Las cargas **positivas** deben quedar en los átomos **menos electronegativos**.
6. Cargas del **mismo signo** sobre átomos adyacentes son **poco probables**.

**Explicación:** la **carga formal** es la herramienta para elegir la mejor estructura entre varias posibles. La fórmula: a los electrones de valencia del átomo libre le restás los que "le tocan" en la molécula = **pares libres (todos sus electrones) + un electrón por cada enlace** (o sea, restás `pares libres × ... ` — ojo: se resta el nº de electrones libres **más** el nº de enlaces, contando cada enlace como que aporta 1 electrón al átomo). La estructura más estable es la que tiene **cargas formales más chicas** y con la carga negativa sobre el átomo **más electronegativo**. Esto justifica por qué el BF₃ no fuerza el doble enlace (pondría carga + sobre el F, que es lo más electronegativo → prohibido por la regla 4/5).
...
**¿Cuál es la más probable?** Analizando con las reglas 3–6: como el **N es más electronegativo** que C y S, la carga negativa debería quedar sobre el **N**, y con la **mínima separación de cargas**. La estructura **(b)** (cargas: N = −1, C = 0, S = 0) es la más representativa.
...
## P3 — pág. 1 · Híbridos de resonancia — Ion nitrato NO₃⁻
...
**Explicación:** el ejemplo canónico de **resonancia**. El NO₃⁻ tiene un doble enlace y dos simples, pero **no hay razón para que el doble esté en un O y no en otro** → se dibujan las **tres** estructuras equivalentes conectadas por ↔. La molécula real es el **híbrido**: los tres enlaces N–O son **idénticos** (intermedios entre simple y doble), y la carga negativa está **repartida** entre los tres O (cada uno con −⅔). La flecha ↔ significa "la molécula es un promedio de estas tres", **no** que oscile entre ellas.

---

## P3 — pág. 2 · Híbridos de resonancia — SO₂
...
1. **Contar** electrones de valencia totales (sumar si carga −, restar si carga +). Dividir /2 = pares.
2. **Átomo central** = el menos electronegativo (H y F siempre afuera).
3. **Unir** central con periféricos por enlaces simples.
4. **Completar octetos** de los periféricos con pares libres.
5. **Sobrantes** al átomo central.
6. **Verificar** el central: si no llega a 8, pasar un par libre de un periférico a **doble/triple enlace**.
7. **Resonancia:** dibujar todas las estructuras equivalentes (↔).
8. **Iones:** entre **corchetes** con la carga afuera.
...
- Suma de CF = 0 (molécula) o = carga del ion.
- Mejor estructura = **CF mínimas**, carga **negativa** en el **más electronegativo**, mínima separación de cargas, sin cargas iguales adyacentes.

## 5. Geometría — TRePEV

**Método:** Lewis → contar **regiones** en el central (enlaces + pares libres; múltiple = **1 región**) → geometría **electrónica** → esconder pares libres → geometría **molecular**.

⚠️ Par libre **empuja más** que enlace → cierra ángulos (109,5° CH₄ > 107,3° NH₃ > 104,5° H₂O).
...
Varias estructuras que **sólo difieren en dónde están los electrones** (misma conectividad), unidas por **↔**. La molécula real es el **híbrido** (un promedio). Ejemplos: **NO₃⁻** (3 estructuras), **SO₂** (2 estructuras). La flecha ↔ **no** es una reacción.
